...
// weekStartStr : ISO date "YYYY-MM-DD" of the week being checked.
//   → pass explicitly for historical weeks ; omit for current week.
// activities   : only available for current week (history endpoint omits them).
//   → activity-level badges (marathon, early_bird, nocturnal, diamond) are
//     skipped when the list is absent.
export async function checkPerformanceBadges(athleteId, weekData, weekStartStr = null) {
  try {
    const { totals, activities = [] } = weekData;
    const currentWeekStart = getWeekStart().toISOString().slice(0, 10);
    const weekStart = weekStartStr ?? currentWeekStart;
    ...
    // ── Activity-level badges — only when the activity list is available ───────
    if (activities.length > 0) {
      ...
      if (activities.some(act => parseInt(act.start_date_local?.slice(11, 13) ?? '12', 10) < 6)) {
        checks.push(awardBadge(athleteId, 'early_bird', weekStart));
      }
      // Nocturnal — start at or after 21:00 local time
      if (activities.some(act => parseInt(act.start_date_local?.slice(11, 13) ?? '0', 10) >= 21)) {
    ...
    // ── Streak ────────────────────────────────────────────────────────────────
    // Always mark this week as active (historical or current).
    if (totals.count > 0) {
      await redis.set(`active_week:${athleteId}:${weekStart}`, '1', { ex: 35 * 86_400 });
    }

    // Only evaluate the streak badge from the current week to avoid
    // awarding it prematurely while historical checks are still running.
    if (isCurrentWeek && totals.count > 0) {
