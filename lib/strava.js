...
function processActivities(activities, weekStart, { includeList = false } = {}) {
  ...
  if (includeList) {
    data.activities = activities.map(({ name, sport_type, type, distance, moving_time, total_elevation_gain, start_date_local, average_temp }) => ({
...
// Cache key prefix — bump version to invalidate stale pre-filter cache entries
