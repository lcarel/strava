...
function processActivities(activities, weekStart, { includeList = false } = {}) {
  ...
  if (includeList) {
    data.activities = activities.map(({ name, sport_type, type, distance, moving_time, total_elevation_gain, start_date, start_date_local, average_temp, average_speed, athlete_count }) => ({
...
const CACHE_KEY_PREFIX_MONTH = 'v3:stats:month'; // v3 — ajout average_speed + athlete_count
