export const CHALLENGES = [
  ...
  { id: 'run_50km',  emoji: '🏃', label: '50 km de course',   desc: 'Cumuler 50 km en courant cette semaine',  type: 'distance', target: 50000,  sport: 'Run' },
  ...
  { id: 'run_longest_42',  emoji: '💀', label: 'Marathon solo',               desc: 'Réaliser une seule course d\'au moins 42,2 km',       type: 'longest_run', target: 42195 },

  // ── Vélo — cumul ──────────────────────────────────────────────────────────
  { id: 'ride_50km',  emoji: '🚴', label: 'Vélo 50 km',   desc: 'Parcourir 50 km à vélo cette semaine',   type: 'distance', target: 50000,  sport: 'Ride' },
  { id: 'ride_100km', emoji: '🚴', label: 'Vélo 100 km',  desc: 'Parcourir 100 km à vélo cette semaine',  type: 'distance', target: 100000, sport: 'Ride' },
  { id: 'ride_150km', emoji: '🚴', label: 'Vélo 150 km',  desc: 'Parcourir 150 km à vélo cette semaine',  type: 'distance', target: 150000, sport: 'Ride' },
  { id: 'ride_3x',    emoji: '🚴', label: '3 sorties vélo', desc: 'Faire au moins 3 sorties à vélo',      type: 'count',    target: 3,      sport: 'Ride' },

  // ── Vélo — records ────────────────────────────────────────────────────────
  { id: 'ride_longest_80',  emoji: '🚵', label: 'Sortie vélo ≥ 80 km',  desc: 'Réaliser une seule sortie vélo d\'au moins 80 km',  type: 'longest_ride', target: 80000 },
  { id: 'ride_longest_130', emoji: '🚵', label: 'Sortie vélo ≥ 130 km', desc: 'Réaliser une seule sortie vélo d\'au moins 130 km', type: 'longest_ride', target: 130000 },

  // ── Toutes activités — cumul ──────────────────────────────────────────────
  ...
  { id: 'any_7x',    emoji: '⚡', label: '7 activités',             desc: 'Une activité par jour de la semaine !', type: 'count',    target: 7,      sport: null },
  ...
  { id: 'climb_2000', emoji: '🏔️', label: '2000 m+ en une sortie', desc: 'Faire 2000 m de dénivelé en une seule activité', type: 'single_elevation', target: 2000 },
  ...
  { id: 'session_5h', emoji: '🐢', label: '5h+ en une seule sortie', desc: 'Tenir au moins 5h de mouvement dans une seule activité', type: 'long_session', target: 18000 },
  ...
  { id: 'days_7',       emoji: '📅', label: 'Bouger tous les jours', desc: 'Une activité chaque jour de la semaine, sans exception',  type: 'active_days', target: 7 },
  { id: 'multi_sport',  emoji: '🎨', label: 'Touche-à-tout',        desc: 'Pratiquer au moins 3 sports différents cette semaine',    type: 'multi_sport',  target: 3 },
  ...
  { id: 'early_bird_3', emoji: '🌅', label: '3× Lève-tôt', desc: 'Sortir avant 7h du matin 3 fois',   type: 'early_bird', target: 3 },
  ...
  { id: 'hot_workout', emoji: '☀️', label: 'Activité par temps chaud', desc: 'S\'entraîner par plus de 25°C',                              type: 'hot_workout', target: 1 },
...
export const CHALLENGE_CATEGORIES = [
  ...
  { label: 'Vélo',            emoji: '🚴', ids: ['ride_50km', 'ride_100km', 'ride_150km', 'ride_3x', 'ride_longest_80', 'ride_longest_130'] },
...
export function computeProgress(stats, challenge) {
  ...
  switch (type) {
    ...
    // Best single activity — distance
    case 'longest_run':
      value = Math.max(0, ...acts.filter(a => (a.sport_type || a.type) === 'Run').map(a => a.distance ?? 0));
      break;

    case 'longest_ride':
      value = Math.max(0, ...acts.filter(a => (a.sport_type || a.type) === 'Ride').map(a => a.distance ?? 0));
      break;
    ...
    // Run with temperature < 5°C (requires device temp sensor)
    case 'cold_run':
      value = acts.filter(a =>
        (a.sport_type || a.type) === 'Run' &&
        a.average_temp !== null &&
        a.average_temp < 5
      ).length;
      break;
